...
func TestArgumentParsing(t *testing.T) {
	...
	t.Run("Test with executable arguments returns 200 and returns executable arguments passed", func(t *testing.T) {
		...
		defer func() { os.Args = oldArgs }()
		flag.CommandLine = flag.NewFlagSet("flag", flag.ExitOnError)
		os.Args = []string{"main.exe", "-host", "remotehost", "-username", "thisismyusername", "-password", "thisismypassword", "-executable", "/path/to/executable", "-script", "README.md", "-executableArg", "arg1", "-executableArg", "arg2"}
		httpClient := httpclient.NewMockHTTPClient(`{"output": "Test output", "exitcode": 2}`, 200)
		var buf bytes.Buffer
		actualExit := invokeClient(&buf, httpClient)
		actualOutput := buf.String()
	...
	t.Run("Test with script arguments returns 200 and returns script arguments passed", func(t *testing.T) {
		...
		flag.CommandLine = flag.NewFlagSet("flag", flag.ExitOnError)
		os.Args = []string{"main.exe", "-host", "remotehost", "-username", "thisismyusername", "-password", "thisismypassword", "-executable", "/path/to/executable", "-script", "README.md",
			"-executableArg", "arg1", "-executableArg", "arg2", "--", "scriptarg1", "-scriptarg scriptarg2", "-scriptarg", "scriptarg3", "--warning=3"}
		httpClient := httpclient.NewMockHTTPClient(`{"output": "Test output", "exitcode": 2}`, 200)
		var buf bytes.Buffer
		actualExit := invokeClient(&buf, httpClient)
		actualOutput := buf.String()
